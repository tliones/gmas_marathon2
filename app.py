...
def render_origin_controls(hotels: pd.DataFrame) -> tuple[Optional[Tuple[float, float, str]], Optional[str]]:
    ...
    if mode == "Choose hotel/lodging":
        ...
        st.sidebar.caption(f"Using starter coordinates for {label}. Verify before public launch.")
...
def main() -> None:
    ...
    st.caption(
        "A GitHub-ready starter Streamlit app for helping visitors find the closest race-day bus pickup from a hotel, address, or GPS point."
    )
    ...
    with tab_map:
        st.subheader("Map of pickup spots and starter hotel data")
        ...
        st.caption(
            "Hotel points are starter data from data/hotels.csv. Review coordinates and add/remove hotels before sharing publicly."
        )
    ...
    with tab_pickups:
        ...
        for _, row in pickups.iterrows():
            with st.expander(row["name"], expanded=False):
                st.write(f"**Address:** {row['full_address']}")
                st.link_button("Open location in Google Maps", google_maps_place_url(row["full_address"]))
